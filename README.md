...
## Общая последовательность действий
  ...
  5. Реализовать примитивную версию алгоритма Median Flow для объекта,
     выделенного окаймляющим прямоугольником на изображении. Общая схема
     для реализации выглядит следующим образом:
     1. Набросать множество точек на первом изображении (в прямоугольнике,
        содержащем интересующий объект).
     2. Протрекерить эти точки на следующий кадр, используя optical flow.
     3. Отфильтровать "плохие" точки.
     4. Восстановить положение прямоугольника на следующем кадре с учетом
